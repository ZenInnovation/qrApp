...
app.service('FeedList', function ($rootScope, FeedLoader, $q) {
    this.get = function(num) {
        ...
        var feedSources = [
            {title: 'Time', url: 'http://ezymekah.com/app/byv_app/'},
            {title: 'Mashable', url: 'http://ezymekah.com/app/byv_app/'},
            {title: 'Wordpress', url: 'http://ezymekah.com/app/byv_app/feed/'}
        ];
        if (feeds.length === 0) {
...
app.service('Blog', function($http) {
    var address = 'ezymekah.com/app/byv_app/';
...
app.service('Firebase', function() {
    this.url = function(id) {
        var url = "https://ringmy-e683b.firebaseapp.com";
        return url;
    };
});
